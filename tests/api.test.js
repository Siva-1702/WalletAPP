...
const startServer = async () => {
  const app = createApp();
  await new Promise((resolve) => app.listen(3100, resolve));
  return app;
};
...
test('health endpoint responds successfully', async () => {
  ...
  const response = await fetch('http://127.0.0.1:3100/api/v1/health');
...
test('OTP registration and wallet flow works end-to-end', async () => {
  ...
  const otpResponse = await fetch('http://127.0.0.1:3100/api/v1/auth/otp/request', {
    method: 'POST',
  ...
  const verifyResponse = await fetch('http://127.0.0.1:3100/api/v1/auth/otp/verify', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  const addMoneyResponse = await fetch('http://127.0.0.1:3100/api/v1/wallet/add-money', {
    method: 'POST',
  ...
  const withdrawResponse = await fetch('http://127.0.0.1:3100/api/v1/wallet/withdraw', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const passbookResponse = await fetch('http://127.0.0.1:3100/api/v1/wallet/passbook', {
  ...
  const kycResponse = await fetch('http://127.0.0.1:3100/api/v1/kyc', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
